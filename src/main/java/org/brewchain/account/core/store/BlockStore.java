...
//											+ loopBlockEntity.getHeader().getStateRoot());
		...
		if (unStableStore.add(block)) {
			...
			if (oParentNode == null && block.getHeader().getNumber() == 1) {
				...
				if (oParentNode == null) {
					...
					// log.warn("parent node number is wrong::" +
					// oParentNode.getNumber());
					oBlockStoreSummary.setBehavior(BLOCK_BEHAVIOR.CACHE);
				} else if (oParentNode != null && !oParentNode.isConnect()) {
					// log.warn("parent node not connect hash::" +
					// oParentNode.getBlockHash() + " number::"
					// + oParentNode.getNumber());
	...
	public synchronized BlockStoreSummary tryAddBlock(BlockEntity block) {
		...
		// log.debug("try to find hash::" + block.getHeader().getParentHash() +
		// " number::"
		// + (block.getHeader().getNumber() - 1));
		...
		if (oParentNode == null) {
			...
			if (existsParent.size() > 0) {
				// log.warn("forks, number::" + (block.getHeader().getNumber() -
				// 1));
				oBlockStoreSummary.setBehavior(BLOCK_BEHAVIOR.EXISTS_PREV);
			} else {
				oBlockStoreSummary.setBehavior(BLOCK_BEHAVIOR.DROP);
	...
	public synchronized BlockStoreSummary connectBlock(BlockEntity block) throws BlockNotFoundInStoreException {
		...
		String hash = block.getHeader().getBlockHash();
		long number = block.getHeader().getNumber();
		// log.debug("connect block number::" + block.getHeader().getNumber() +
		// " hash::" + hash + " stateroot::"
	...
	public  BlockEntity getBlockByNumber(long number) {
		BlockEntity oBlockEntity = stableStore.getBlockByNumber(number);
		if (oBlockEntity == null) {
			oBlockEntity = unStableStore.getBlockByNumber(number);
		}
		return oBlockEntity;
	}
	...
	public  List<BlockEntity> getChildListBlocksEndWith(String blockHash, String endBlockHash,
			int maxCount) {
	...
	public  List<BlockEntity> getParentListBlocksEndWith(String blockHash, String endBlockHash,
			long maxCount) {
		BlockEntity firstBlock = getBlockByHash(blockHash);
		if (firstBlock == null) {
			return new ArrayList<>();
	...
	public synchronized BlockEntity rollBackTo(long number) {
		// log.info("blockstore try to rollback to number::" + number + "
		// maxconnect::" + this.getMaxConnectNumber()
