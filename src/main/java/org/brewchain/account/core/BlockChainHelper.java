...
public class BlockChainHelper implements ActorService {
	...
	public String getNodeAccount() {
		OValue oOValue;
		try {
			oOValue = dao.getAccountDao().get(oEntityHelper.byteKey2OKey(blockChainConfig.getNodeAccount().getBytes()))
					.get();
				...
				if (blockChainConfig.isDev()) {
					...
					try {
						...
						if (oKeyStoreValue == null) {
							...
							dao.getAccountDao().put(oEntityHelper.byteKey2OKey(blockChainConfig.getNodeAccount().getBytes()),
