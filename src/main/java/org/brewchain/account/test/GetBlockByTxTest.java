...
public class GetBlockByTxTest extends SessionModules<ReqTxTest> implements ActorService {
	...
	public void onPBPacket(final FramePacket pack, final ReqTxTest pb, final CompleteHandler handler) {
		...
		ByteString txHash;
		try {
			accountHelper.CreateAccount(encApi.hexDec(oKeyPairs1.getAddress()), encApi.hexDec(oKeyPairs1.getPubkey()));
			accountHelper.CreateAccount(encApi.hexDec(oKeyPairs2.getAddress()), encApi.hexDec(oKeyPairs2.getPubkey()));

			accountHelper.addBalance(encApi.hexDec(oKeyPairs1.getAddress()), 100000);
			accountHelper.addBalance(encApi.hexDec(oKeyPairs2.getAddress()), 100000);
			// 创建几个交易
		...
		try {
			BlockEntity oBlockEntity = blockHelper.getBlockByTransaction(txHash.toByteArray());
			log.debug(String.format("===> block ", oBlockEntity.getHeader().getBlockHash().toStringUtf8()));
		} catch (Exception e) {
